...
def main():
    ...
    if args.log:
        if os.path.isdir(args.log):
            tm = datetime.now()
            logfile = os.path.join(args.log,"package_setsm_{}.log".format(tm.strftime("%Y%m%d%H%M%S")))
        else:
    ...
    if os.path.isfile(src):
        ...
        try:
            ...
            if not os.path.isfile(raster.archive) or args.overwrite or args.filter_dems:
            ...
            for f in files:
                if f.endswith("_dem.tif") and "m_" in f:
                    ...
                    try:
                        ...
                        if not os.path.isfile(raster.archive) or not os.path.isfile(raster.mdf) or not os.path.isfile(raster.readme)or args.overwrite or args.filter_dems:
                            rasters.append(raster)
