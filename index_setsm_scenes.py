...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="build setsm scene DEM index"
        )
    ...
    if args.log:
        if os.path.isdir(args.log):
            tm = datetime.now()
            logfile = os.path.join(args.log,"index_setsm_scenes_{}.log".format(tm.strftime("%Y%m%d%H%M%S")))
        else:
    ...
    if os.path.isfile(src):
        logger.info(src)
        try:
            raster = dem.SetsmScene(os.path.join(src))
        except RuntimeError, e:
            logger.error( e )
        else:
        ...
        for root,dirs,files in os.walk(src):
            for f in files:
                if f.endswith("meta.txt"):
                    ...
                    try:
                        raster = dem.SetsmScene(os.path.join(root,f))
                    except RuntimeError, e:
                        logger.error( e )
                    else:
                        rasters.append(raster)
    ...
    if total > 0:
        ...
        if ds is not None:
            ...
            if layer is not None:
                ...
                for field_def in utils.SCENE_ATTRIBUTE_DEFINITIONS:
                ...
                for raster in rasters:
                    ...
                    try:
                        ...
                        ## Set fields
                        feat.SetField("SCENE_ID",raster.sceneid)
                        ...
                        feat.SetField("HAS_LSF",int(os.path.isfile(raster.lsf_dem)))
                        feat.SetField("HAS_NONLSF",int(os.path.isfile(raster.dem)))

                        ## If lsf, use that path for DEM name, otherwise non-lsf dem
                        if os.path.isfile(raster.lsf_dem):
                            dem_name = raster.lsf_dem
                        else:
                            dem_name = raster.dem
                        feat.SetField("DEM_NAME",dem_name)
                        ...
                        feat.SetField("FILESZ_LSF",raster.filesz_lsf)
